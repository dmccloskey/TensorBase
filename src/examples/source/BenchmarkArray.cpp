/**TODO:  Add copyright*/

#include <unsupported/Eigen/CXX11/Tensor>
#include <TensorBase/benchmarks/BenchmarkArrayDefaultDevice.h>

using namespace TensorBase;
using namespace TensorBaseBenchmarks;

/* Benchmark for the TensorArray fixed-size array class

Example usage:
	BenchmarkArray [data_size] [array_size] [n_engines]
	BenchmarkArray S S 8

@param[in] data_size Options include XS, S, M, L, and XL (i.e., 10, 1e3, 1e6, 1e9, and 1e12, respectively) with default of S
@param[in] data_size Options include XS, S, M, L, and XL (i.e., 8, 32, 128, 512, and 2048, respectively) with default of S
@param[in] n_engines The number of engines (i.e., threads for CPU and streams for GPU) to use (only available on CPU and GPU versions)
*/
int main(int argc, char** argv)
{
	// Parse the user commands
	int data_size = 1e3;
  int array_size = 32;
  int n_engines = 1;
  parseCmdArgs(argc, argv, data_size, array_size, n_engines);

  // Setup the benchmarking suite
  BenchmarkArrayDefaultDevice<char> benchmark_array;

	// Setup the device
	Eigen::DefaultDevice device;

	// run the application
  runBenchmarkArray(data_size, array_size, benchmark_array, device);

	return 0;
}