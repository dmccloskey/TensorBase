...
int main(int argc, char** argv)
{
	...
	//Benchmark1TimePointCpu<int, int> benchmark_1_tp; // 0D only

	// Setup the TensorCollectionGenerator
	TensorCollectionGeneratorCpu<int, float> tensor_collection_generator;
	//TensorCollectionGeneratorCpu<int, int> tensor_collection_generator; // 0D only

	// Setup the device
  Eigen::ThreadPool pool(8);
  Eigen::ThreadPoolDevice device(&pool, 8);
