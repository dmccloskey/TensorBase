...
std::string Benchmark1TimePoint<LabelsT, TensorT, DeviceT>::insert1TimePoint(const int& n_dims, TransactionManager<DeviceT>& transaction_manager, const int& data_size, DeviceT& device)
{
	...
	if (n_dims == 0) {
		...
		insert1TimePoint2D(transaction_manager, data_size, device);
	}
	else if (n_dims == 3) {
		// TODO...
	}
...
template<typename LabelsT, typename TensorT, typename DeviceT>
static void run_pixel_benchmark(const std::string& data_dir, const int& n_dims, const int& data_size, const bool& in_memory, const double& shard_span_perc, 
