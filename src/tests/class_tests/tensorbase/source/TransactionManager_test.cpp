...
BOOST_AUTO_TEST_CASE(undoRedoAndRollbackDefaultDevice)
{
  ...
  // Set up the update
  TensorUpdateSelectTensorData<char, Eigen::DefaultDevice, 1> tensorUpdate("3", SelectTable3(), std::make_shared<TensorDataDefaultDevice<char, 1>>(values_update));
  std::shared_ptr<TensorOperation<Eigen::DefaultDevice>> tensorUpdate_ptr = std::make_shared<TensorUpdateSelectTensorData<char, Eigen::DefaultDevice, 1>>(tensorUpdate);
