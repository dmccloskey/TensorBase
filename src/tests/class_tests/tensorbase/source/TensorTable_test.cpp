...
BOOST_AUTO_TEST_CASE(adjustSliceIndicesToDataSizeDefaultDevice)
{
  // setup the table
  TensorTableDefaultDevice<float, 3> tensorTable;
  Eigen::DefaultDevice device;
  ...
  //std::map<int, std::pair<Eigen::array<Eigen::Index, 3>, Eigen::array<Eigen::Index, 3>>> slice_indices;
  //Eigen::array<Eigen::Index, 3> shard_data_dimensions;
  //int shard_data_size = tensorTable.makeSliceIndicesFromShardIndices(shard_id_indices_ptr, slice_indices, shard_data_dimensions, device);
  //std::cout << "shard_data_size\n" << shard_data_size << std::endl;
  //for (auto& slice_index : slice_indices) {
  //  std::cout << "slice_indices_test.emplace(" << slice_index.first << ", std::make_pair(Eigen::array<Eigen::Index, 3>({";
  //  for (int i = 0; i < 3; ++i) {
  //    std::cout << slice_index.second.first.at(i) << ",";
  //  }
  //  std::cout << " }), Eigen::array<Eigen::Index, 3>({ ";
  //  for (int i = 0; i < 3; ++i) {
  //    std::cout << slice_index.second.second.at(i) << ",";
  //  }
  //  std::cout << "})));" << endl;
  //}
