...
void test_InsertUpdateDeleteGpu()
{
  ...
  gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_weights")->getDataTensorSize(), 0);
  gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_node_property")->getAxes().at("1_nodes")->getNDimensions(), 1);
  //gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_node_property")->getAxes().at("1_nodes")->getNLabels(), 0); FIXME
  //gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_node_property")->getDataTensorSize(), 0); FIXME
  gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_link_property")->getAxes().at("1_links")->getNDimensions(), 1);
  //gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_link_property")->getAxes().at("1_links")->getNLabels(), 0); FIXME
  //gpuCheckEqual(n_dim_tensor_collection->tables_.at("Graph_link_property")->getDataTensorSize(), 0); FIXME
