...
namespace TensorBase
{
  ...
  protected:
    ...
    void getUniqueIds(const std::shared_ptr<TensorData<LabelsT, Eigen::GpuDevice, 2>>& indices, std::shared_ptr<TensorData<LabelsT, Eigen::GpuDevice, 1>>& node_ids, Eigen::GpuDevice& device) const override;
  };
  template<typename LabelsT, typename TensorT>
  ...
  template<typename LabelsT, typename TensorT>
  inline void KroneckerGraphGeneratorGpu<LabelsT, TensorT>::getUniqueIds(const std::shared_ptr<TensorData<LabelsT, Eigen::GpuDevice, 2>>& indices, std::shared_ptr<TensorData<LabelsT, Eigen::GpuDevice, 1>>& node_ids, Eigen::GpuDevice& device) const
  {
    // Sort a copy of the data
    TensorDataGpuPrimitiveT<LabelsT, 1> indices_tmp(Eigen::array<Eigen::Index, 1>({ (int)indices->getTensorSize() }));
    indices_tmp.setData();
    indices_tmp.syncHAndDData(device);
