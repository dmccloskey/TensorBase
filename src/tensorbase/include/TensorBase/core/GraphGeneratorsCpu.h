...
namespace TensorBase
{
  ...
  protected:
    ...
    void getUniqueIds(const std::shared_ptr<TensorData<LabelsT, Eigen::ThreadPoolDevice, 2>>& indices, std::shared_ptr<TensorData<LabelsT, Eigen::ThreadPoolDevice, 1>>& node_ids, Eigen::ThreadPoolDevice& device) const override;
  };
  template<typename LabelsT, typename TensorT>
  ...
  template<typename LabelsT, typename TensorT>
  inline void KroneckerGraphGeneratorCpu<LabelsT, TensorT>::getUniqueIds(const std::shared_ptr<TensorData<LabelsT, Eigen::ThreadPoolDevice, 2>>& indices, std::shared_ptr<TensorData<LabelsT, Eigen::ThreadPoolDevice, 1>>& node_ids, Eigen::ThreadPoolDevice& device) const
  {
    // Sort a copy of the data
    TensorDataCpu<LabelsT, 1> indices_tmp(Eigen::array<Eigen::Index, 1>({ (int)indices->getTensorSize() }));
    indices_tmp.setData();
    indices_tmp.syncHAndDData(device);
